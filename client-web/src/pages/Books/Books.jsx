...
const Books = () => {
  ...
  const [cardType, setCardType] = useState("rectangle");
  ...
  const allAuthors = Array.from(new Set(booksData.map((book) => book.author)));
  const allCategories = Array.from(
    new Set(booksData.map((book) => book.category).filter(Boolean)),
  );
  const allCollections = Array.from(
    new Set(booksData.map((book) => book.collection).filter(Boolean)),
  );
  ...
  return (
    <div
      className={`container mx-auto p-4 space-y-6 ${theme.background.section}`}
    >
      {/* Search & Controls Bar */}
      ...
      <div className="text-sm text-gray-600">
        ...
        {(selectedTags.length > 0 ||
          ...
          selectedCategories.length > 0 ||
          selectedCollections.length > 0 ||
          selectedSubjects.length > 0) && <span> matching your filters</span>}
      </div>
